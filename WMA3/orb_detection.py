...
def find_best_match(kp_frame, des_frame, keypoints_and_descriptors):
    ...
    good_matches = [[m for m in match if m.distance < 0.8 * np.mean([n.distance for n in match])] for match in matches]
...
def main():
    ...
    while True:
        ...
        kp_frame, des_frame = cv.ORB_create().detectAndCompute(gray_frame, None)
        ...
        if len(very_good) >= MIN_MATCH_COUNT:
            src_pts = np.float32(
                [keypoints_and_descriptors[best_matched_image_index][0][m.queryIdx].pt for m in very_good]).reshape(-1,
                                                                                                                    1,
                                                                                                                    2)
        ...
        img4 = cv.drawMatches(best_matched_image, keypoints_and_descriptors[best_matched_image_index][0], frame,
                              kp_frame, very_good, None, **draw_params)
